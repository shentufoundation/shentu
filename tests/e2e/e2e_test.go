...
func (s *IntegrationTestSuite) TestOracle() {
	...
	var atxHash2, taskHash2 string
	var atxHash3, taskHash3 string
	var err error
	...
	s.Run("create_atx_task", func() {
		...
		// 0 score task
		atxHash2, err = s.executeOracleCreateAtxTask(s.chainA, 0, txBytes2, chainID, bounty.String(), valTimeStr, alice.String(), feesAmountCoin.String())
		s.Require().NoError(err)
		s.Require().Eventually(
			func() bool {
				res, e := queryOracleTaskHash(chainAAPIEndpoint, atxHash2)
				if e == nil {
					taskHash2 = res
					return true
				}
		...
		// long term task
		atxHash3, err = s.executeOracleCreateAtxTask(s.chainA, 0, txBytes3, chainID, bounty.String(), longValTimeStr, alice.String(), feesAmountCoin.String())
		s.Require().NoError(err)
		s.Require().Eventually(
			func() bool {
				res, e := queryOracleTaskHash(chainAAPIEndpoint, atxHash3)
				if e == nil {
					taskHash3 = res
					return true
				}
	...
	s.Run("respond_atx_task", func() {
		...
		s.executeOracleRespondAtxTask(s.chainA, 0, 60, taskHash, charle.String(), feesAmountCoin.String())
		s.Require().Eventually(
			func() bool {
				res, e := queryOracleAtxTask(chainAAPIEndpoint, taskHash)
				s.Require().NoError(e)
		...
		s.executeOracleRespondAtxTask(s.chainA, 0, 0, taskHash2, alice.String(), feesAmountCoin.String())
		s.executeOracleRespondAtxTask(s.chainA, 0, 0, taskHash2, bob.String(), feesAmountCoin.String())

		s.executeOracleRespondAtxTask(s.chainA, 0, 70, taskHash3, charle.String(), feesAmountCoin.String())
	})
	...
	s.Run("close_atx_task", func() {
		...
		s.Require().Eventually(
			func() bool {
				res, e := queryOracleAtxTask(chainAAPIEndpoint, taskHash2)
				s.Require().NoError(e)
		...
		s.Require().Eventually(
			func() bool {
				res, e := queryOracleAtxTask(chainAAPIEndpoint, taskHash3)
				s.Require().NoError(e)
