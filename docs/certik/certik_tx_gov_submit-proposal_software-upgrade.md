## certik tx gov submit-proposal software-upgrade
...
```
certik tx gov submit-proposal software-upgrade [name] (--upgrade-height [height] | --upgrade-time [time]) (--upgrade-info [info]) [flags]
```
