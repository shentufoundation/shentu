...
`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			msg := types.NewMsgCreatePool(fromAddr, shield, nativeDeposit, nil, sponsor, sponsorAddr, description, shieldLimit)
			if err := msg.ValidateBasic(); err != nil {
				return err
			}
...
`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			msg := types.NewMsgUpdatePool(fromAddr, shield, nativeDeposit, nil, id, description, shieldLimit)
			if err := msg.ValidateBasic(); err != nil {
				return err
			}
