...
`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			deposits, err := sdk.ParseCoinsNormalized(viper.GetString(flagNativeDeposit))
			if err != nil {
				return err
			}
			...
			msg := types.NewMsgCreatePool(fromAddr, shield, deposits, sponsor, sponsorAddr, description, shieldLimit)
			if err := msg.ValidateBasic(); err != nil {
				return err
			}
...
`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			msg := types.NewMsgUpdatePool(fromAddr, shield, deposits, id, description, shieldLimit)
			if err := msg.ValidateBasic(); err != nil {
				return err
			}
