...
func TestMsgUpdateCertifier(t *testing.T) {
	...
	addFirst := types.NewMsgUpdateCertifier(authority, addrs[0], "first certifier", types.Add, addrs[1])
	_, err := msgServer.UpdateCertifier(sdk.WrapSDKContext(ctx), addFirst)
	require.NoError(t, err)

	certifier, err := app.CertKeeper.GetCertifier(ctx, addrs[0])
	require.NoError(t, err)
	require.Equal(t, types.NewCertifier(addrs[0], addrs[1], "first certifier"), certifier)

	_, err = msgServer.UpdateCertifier(sdk.WrapSDKContext(ctx), addFirst)
	require.True(t, errorsmod.IsOf(err, types.ErrCertifierAlreadyExists), "expected ErrCertifierAlreadyExists, got %v", err)

	addSecond := types.NewMsgUpdateCertifier(authority, addrs[2], "second certifier", types.Add, addrs[3])
	_, err = msgServer.UpdateCertifier(sdk.WrapSDKContext(ctx), addSecond)
	require.NoError(t, err)

	removeFirst := types.NewMsgUpdateCertifier(authority, addrs[0], "", types.Remove, addrs[1])
	...
	removeLast := types.NewMsgUpdateCertifier(authority, addrs[2], "", types.Remove, addrs[3])
...
func TestMsgUpdateCertifierRejectsUnauthorizedAuthority(t *testing.T) {
	...
	msg := types.NewMsgUpdateCertifier(addrs[0], addrs[1], "", types.Add, nil)
...
func TestCertifierQueryByAddress(t *testing.T) {
	...
	expected := types.NewCertifier(addrs[0], addrs[1], "query me")
