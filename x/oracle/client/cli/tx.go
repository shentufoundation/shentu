...
func GetCmdCreateTask(cdc *codec.Codec) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if expirationStr != "" {
				expiration, err = time.Parse(layout, expirationStr)
				if err != nil {
					return err
				}
			} else {
			...
			msg := types.NewMsgCreateTask(contract, function, bounty, description, expiration, cliCtx.GetFromAddress(), wait)
			if err := msg.ValidateBasic(); err != nil {
				return err
			}
			return utils.GenerateOrBroadcastMsgs(cliCtx, txBldr, []sdk.Msg{msg})
	...
	cmd.Flags().String(FlagExpiration, "", "expiration of the task result")
	cmd.Flags().String(FlagWait, "", "number of blocks between task creation and aggregation")
