...
docker run --name peggy_solidity_test_instance --mount type=bind,source="$(pwd)"/,target=/peggy --cap-add=NET_ADMIN -it peggy-base /bin/bash /shentu/peggy-tests/container-scripts/solidity-tests.sh